...
**本插件独有、cship 没有的功能**：任务栏上还能同时看到每个正在跑的 Claude Code
终端当前是什么状态——🔵 正在思考、🟡 等待你做决定、🟢 已完成、🔴 出错、🔘 空闲，
开着好几个终端也能一眼分辨哪个该看了，不用一个个切窗口去找：

```
● ● ●   ← 三个终端：正在思考 / 等待你做决定 / 已完成
```

这个信息插件本身看不到，靠 Claude Code 的 hooks 主动上报，见下面
「[Claude 终端状态](#claude-终端状态)」一节。
